...
function download(url) {
    return new Promise((S,J)=>{
        ...
        fetch(url, Object.assign({credentials: "same-origin"}))
            .then((res) => {
                //获取响应图片大小
                let responseSize = res.headers.get('content-length');
                logger.debug( `response size ${responseSize}` );
                //报错，直接挂掉
                if (!res.ok){
                    PromiseDone( false, `download photo error,http status: ${res.status} `);
                }
                //保存到文件，返回新的Promise
                else{
                    ...
                    fileStream.on("finish", function() {
                        fileStream.close();
                        fs.stat(saveFile,function( err,stats ){
                            if( err ){
                                logger.error( err );
                                return PromiseDone( false, err );
                            }
                            if( stats['size'].toString() !== responseSize.toString() ){
                                let errStr = `download size error! need ${responseSize},got ${stats['size']}`;
                                return PromiseDone( false,errStr );
                            }
                            else{
                                return PromiseDone( true, saveFile );
                            }
                        });
                    });
                }
...
function randomPhoto(unsplash) {
    return new Promise((S,J)=>{
        ...
        .then((urls) => {
            logger.debug( `quality: ${ _.keys( urls ) }`);
            let url = null;
